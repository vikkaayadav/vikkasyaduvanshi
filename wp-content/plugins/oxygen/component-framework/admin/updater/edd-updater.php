...
add_filter( 'update_plugins_oxygenbuilder.com', function( $update, array $plugin_data, string $plugin_file, $locales ) {
	...
	global $oxygen_updater;
	if (!$oxygen_updater->edd_updater) {
		$oxygen_updater->init();
	}
    $version_info = $oxygen_updater->edd_updater->api_request( 'plugin_latest_version', array( 'slug' => 'functions' ) );

	// check if this is a license for this exact plugin
	if ( !$version_info || $version_info->name !== $oxygen_updater->edd_updater->get_api_data()["item_name"] ) {
		return $update;    
	}
